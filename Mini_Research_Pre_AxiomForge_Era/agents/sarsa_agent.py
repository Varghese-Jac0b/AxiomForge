"""
Tabular SARSA agent for Mini Research World.

Run from the project root:

    python agents/sarsa_agent.py
    python agents/sarsa_agent.py --episodes 5000 --seed 42
    python agents/sarsa_agent.py --episodes 1000 --save-csv results/sarsa_training.csv
    python agents/sarsa_agent.py --episodes 5000 --save-qtable results/sarsa_qtable.pkl
"""
...
def train_sarsa_episode(env, q_table, alpha, gamma, epsilon, rng, render=False):
    """
    Train one SARSA episode and update the Q-table on every step.

    SARSA uses the action actually taken at the next state:
        target = reward + gamma * Q(next_state, next_action)

    At the terminal step, there is no next action:
        target = reward
    ...
    while not terminated and not truncated:
        ...
        if not done:
            # SARSA: bootstrap from the next action we will actually take.
            next_action = epsilon_greedy_action(
                q_table, next_state, env, epsilon, rng
            )
            next_q = get_q_values(q_table, next_state, num_actions)[next_action]
...
def train_sarsa(
    ...
    """
    Train a tabular SARSA agent over many episodes.
    ...
    for episode in range(1, num_episodes + 1):
        ...
        episode_result = train_sarsa_episode(
            env=env,
            q_table=q_table,
            alpha=alpha,
            gamma=gamma,
...
def print_summary(results_df):
    """
    Print a clean SARSA training summary.
    """
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Train a tabular SARSA agent on Mini Research World."
    )
...
def main():
    ...
    q_table, results_df = train_sarsa(
