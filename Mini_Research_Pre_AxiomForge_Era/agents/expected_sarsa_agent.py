"""
Tabular Expected SARSA agent for Mini Research World.

Run from the project root:

    python agents/expected_sarsa_agent.py
    python agents/expected_sarsa_agent.py --episodes 1000 --seed 42
    python -m agents.expected_sarsa_agent --episodes 1000 --seed 42
    python agents/expected_sarsa_agent.py --episodes 1000 --save-csv results/expected_sarsa_training.csv
    python agents/expected_sarsa_agent.py --episodes 5000 --save-qtable results/expected_sarsa_qtable.pkl
"""
...
def expected_q_value(q_table, state, num_actions, epsilon):
    """
    Compute the expected Q-value of a state under the epsilon-greedy policy.

    Each action gets a base probability of epsilon / num_actions.
    The greedy (best) action gets an extra (1 - epsilon).

        policy_prob(a) = epsilon / num_actions          for every action
        policy_prob(best_action) += (1 - epsilon)

        Expected_Q(state) = sum_a policy_prob(a) * Q(state, a)

    This is what makes Expected SARSA different from normal SARSA:
    instead of bootstrapping from the single action actually taken next,
    we average over every action the policy could take, weighted by how
    likely the policy is to take it.
    """

    q_values = get_q_values(q_table, state, num_actions)

    # Start every action off with the exploration share of probability.
    action_probs = np.full(num_actions, epsilon / num_actions, dtype=np.float64)

    # The greedy action gets the remaining (1 - epsilon) probability mass.
    # Ties are split so the probabilities still sum to 1.
    best_value = q_values.max()
    best_actions = np.flatnonzero(q_values == best_value)
...
def train_expected_sarsa_episode(
    env, q_table, alpha, gamma, epsilon, rng, render=False
):
    """
    Train one Expected SARSA episode and update the Q-table on every step.

    Expected SARSA bootstraps from the expected value of the next state
    under the current epsilon-greedy policy:

        target = reward + gamma * Expected_Q(next_state)
    ...
    while not terminated and not truncated:
        ...
        if not done:
            # Expected SARSA: bootstrap from the policy-weighted average
            # of the next state's action values, not a single sampled action.
...
def train_expected_sarsa(
    ...
    """
    Train a tabular Expected SARSA agent over many episodes.
    ...
    for episode in range(1, num_episodes + 1):
        ...
        episode_result = train_expected_sarsa_episode(
            env=env,
            q_table=q_table,
            alpha=alpha,
            gamma=gamma,
...
def print_summary(results_df):
    """
    Print a clean Expected SARSA training summary.
    """
    ...
    print("\n========== EXPECTED SARSA AGENT TRAINING SUMMARY ==========")
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Train a tabular Expected SARSA agent on Mini Research World."
    )
...
def main():
    ...
    q_table, results_df = train_expected_sarsa(
